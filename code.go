...
// cmd is ignored as everything is now highlighted with pygments.
func parseCode(ctx *Context, sourceFile string, sourceLine int, cmd string) (Elem, error) {
	...
	pygments := sandman.Highlight(omitLines(textBytes, lo, hi), strings.ToLower(lang.Language), false)
...
// span the byte range specified by start and end.
// It discards lines that end in "OMIT".
func omitLines(src []byte, start, end int) (lines string) {
	startLine := 1
	for i, b := range src {
		if i == start {
			break
